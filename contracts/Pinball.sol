...
library LibPinball {
    struct State {
        bytes ball;
        uint256 commandsOffset;
        uint256 commandsLength;
        uint256 totalTiltPrice;
        bool missionAvailable;
        uint256 currentMission;
        uint256 currentPowerup;
        uint256 location;
        uint32 rand;
        uint256 baseScore;
    ...
    function readUint8At(State memory state, uint256 dataOffset) internal returns (uint8 result) {
        assembly {
            result := mload(add(add(mload(state), 1), dataOffset))
        }
    }

    function readUint16At(State memory state, uint256 dataOffset) internal returns (uint16 result) {
        assembly {
            result := mload(add(add(mload(state), 2), dataOffset))
        }
    }

    function readBytes4At(State memory state, uint256 dataOffset) internal returns (bytes4 result) {
        assembly {
            result := mload(add(add(mload(state), 0x20), dataOffset))
        }
    }

    function readBytes32At(State memory state, uint256 dataOffset) internal returns (bytes32 result) {
        assembly {
            result := mload(add(add(mload(state), 0x20), dataOffset))
        }
    }
...
contract Pinball {
    ...
    event NewScore(uint256 id, address indexed who, uint256 score);
    ...
    function makeCommitmentHash(bytes32 commitment, uint256 blockNumber) private returns (bytes32) {
    ...
    function play(bytes memory ball, uint256 blockNumber) external {
        ...
        uint256 finalScore = state.baseScore * state.scoreMultiplier + state.bonusScore;
    ...
    function tick(LibPinball.State memory state, uint256 commandNum) private returns (bool) {
        uint256 commandOffset = state.commandsOffset + commandNum * 5;
    ...
    function tilt(
        ...
        uint256 tiltSpend = state.readUint8At(dataOff);
    ...
    function flipLeft(
        ...
        if (selector == 0x50407060) {
            ...
            uint256 location = state.location;
            if (location > 0) {
                ...
                if (checkAmount > 0) {
                    ...
                    for (uint256 i = 0; i < checkAmount; i++) {
        ...
        if (selector == 0x00e100ff) {
            if (!state.missionAvailable) {
                ...
                if (
                    ...
                    hash == 0x38c56aa967695c50a998b7337e260fb29881ec07e0a0058ad892dcd973c016dc
                ) {
                    ...
                    state.currentMission == 1 &&
                    hash == 0x8f038627eb6f3adaddcfcb0c86b53e4e175b1d16ede665306e59d9752c7b2767
                ) {
                    ...
                    state.currentMission == 2 &&
                    hash == 0xfe7bec6d090ca50fa6998cf9b37c691deca00e1ab96f405c84eaa57895af7319
                ) {
                    ...
                    state.currentMission == 3 &&
                    hash == 0x46453a3e029b77b65084452c82951b4126bd91b5592ef3b88a8822e8c59b02e8
                ) {
                    state.missionAvailable = true;
            ...
            uint256 skip = 3 * (state.location - 65);
